...
function points() {
  ...
  text("High Score: " + highScore, 8, 40);
}
...
function restart() {
  ...
  if (active == false) {
    ...
    fill('white');
    text('Click to Restart', width / 2 - 75, height / 2);
    for(i = 0; i < 10; i++) {
      if (isTouched(x, y, w, h, mouseX, mouseY)) {
        setup();
...
function isTouched(x1, y1, w1, h1, testX, testY) {
  let isInsideLeft = testX > x1;
  let isInsideRight = testX < x1 + w1;
  let isInsideTop = testY > y1;
  let isInsideBottom = testY < y1 + h1;
  if (isInsideLeft && isInsideRight && isInsideTop && isInsideBottom && mouseIsPressed) {
    setup();
  }
}